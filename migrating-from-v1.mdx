...
| `GET /v1/catalog/product?sku=` | `GET /v2/catalog/products/{id}` | Fetch by catalog `productId`; see [Product codes](/concepts/product-codes) |
...
<Note>
  ...
  as in v1. Catalog responses now expose Kalixo `productCode` and locale `sku` instead of
  provider identifiers — see [Product codes](/concepts/product-codes). Pricing validation is
  unchanged for fixed products (price must match) and range-checked for
  [open-denomination](/concepts/open-denomination) products.
</Note>
